...
#     # Pass the district name to the market_alignment.html template
#     return render_template('market.html', district=district, crop1=crop_names)
...
#         query = f"SELECT Commodity, Modal_x0020_Price FROM your_table_name WHERE District = ? AND Commodity IN (?, ?, ?) ORDER BY Modal_x0020_Price DESC"
#         cursor.execute(query, (district, crop_names[0], crop_names[1], crop_names[2]))
...
#         sorted_crops = []
#         for row in results:
#             sorted_crops.append({"commodity": row[0], "modal_price": row[1]})
...
def align_to_market():
    ...
    try:
        # Create placeholders for the crop names in the SQL query
        placeholders = ', '.join(['?'] * len(crop_names))

        # Execute a query to select commodity and modal price for the given district and crops
        query = f"SELECT Commodity, Modal_x0020_Price FROM your_table_name WHERE District = ? AND Commodity IN ({placeholders}) ORDER BY Modal_x0020_Price DESC"
        ...
        return render_template('market.html', district=district, crop1=crop_names, crops=sorted_crops, crop_images=crop_images)
